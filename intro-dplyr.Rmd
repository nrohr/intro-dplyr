...
Let's try an example. Suppose we want to examine the number of customers who have positive, negative, or zero net worth for each value of home ownership. To do that, we need to take our raw data and do the following:
...
Great! Now let's make a bar plot of the data so we can compare these numbers visually. There are two ways to do this. One is that we can pipe the code above directly into our code for plotting data using the `ggplot2` package:
